...
pub async fn get_ssm_value(ssm: &SsmClient, name: &str) -> Result<String, String> {
    ssm.get_parameter()
        .name(name)
...
pub async fn wait_for_healthy(http: &reqwest::Client, base_url: &str) -> Result<(), String> {
...
pub async fn handler(event: LambdaEvent<serde_json::Value>) -> Result<serde_json::Value, Error> {
